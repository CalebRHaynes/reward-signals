...
for i = 1:100 % 10 iterations
    ...
    [b, loglike, exitflag, output, ~, ~, H] = fmincon(@model, inx, [],[],[],[],lb,ub,[], options, choice, reward);
...
function [loglike, V, cV, rpe] = model(x, choice, reward)
...
%caleb-
V = zeros(ntrial,3); %columns: 1 (deckchoice1), 2 (deckchoice2)

%V(1,:) = [0 0 0]; %assuming neutral starting point

%caleb-
V(1,:) = [50 50]; %assuming neutral starting point
